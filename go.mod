module github.com/chebizarro/redshed

go 1.14

require (
	github.com/99designs/gqlgen v0.11.3
	github.com/alecthomas/template v0.0.0-20190718012654-fb15b899a751
	github.com/appleboy/gin-jwt v2.5.0+incompatible // indirect
	github.com/appleboy/gin-jwt/v2 v2.6.3
	github.com/casbin/casbin v1.9.1
	github.com/cmelgarejo/go-gql-server v0.0.0-20200316220838-a801cf3fa60b
	github.com/cpuguy83/go-md2man/v2 v2.0.0 // indirect
	github.com/creasty/defaults v1.3.0
	github.com/gin-contrib/cors v1.3.1
	github.com/gin-contrib/sessions v0.0.3
	github.com/gin-gonic/gin v1.5.0
	github.com/globalsign/mgo v0.0.0-20181015135952-eeefdecb41b8
	github.com/go-openapi/spec v0.19.7 // indirect
	github.com/go-openapi/swag v0.19.8 // indirect
	github.com/go-playground/validator/v10 v10.2.0 // indirect
	github.com/gofrs/uuid v3.2.0+incompatible
	github.com/gofrs/uuid/v3 v3.1.2 // indirect
	github.com/golang/protobuf v1.3.3 // indirect
	github.com/graphql-go/graphql v0.7.9 // indirect
	github.com/jinzhu/gorm v1.9.12
	github.com/json-iterator/go v1.1.9 // indirect
	github.com/mailru/easyjson v0.7.1 // indirect
	github.com/markbates/goth v1.63.0
	github.com/mattn/go-isatty v0.0.12 // indirect
	github.com/paulmach/orb v0.1.5
	github.com/pkg/errors v0.9.1 // indirect
	github.com/sirupsen/logrus v1.5.0
	github.com/spf13/viper v1.6.2
	github.com/swaggo/gin-swagger v1.2.0
	github.com/swaggo/swag v1.6.5
	github.com/urfave/cli v1.22.3 // indirect
	github.com/vektah/gqlparser/v2 v2.0.1
	golang.org/x/crypto v0.0.0-20191205180655-e7c4368fe9dd
	golang.org/x/net v0.0.0-20200324143707-d3edc9973b7e // indirect
	golang.org/x/tools v0.0.0-20200327170918-eca45d481ddc // indirect
	gopkg.in/dgrijalva/jwt-go.v3 v3.2.0 // indirect
	gopkg.in/gormigrate.v1 v1.6.0
	gopkg.in/yaml.v2 v2.2.8 // indirect
)
